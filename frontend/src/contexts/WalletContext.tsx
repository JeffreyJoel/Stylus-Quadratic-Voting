...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  const connect = useCallback(async (requestedWalletName?: string) => {
    ...
    try {
      ...
      if (requestedWalletName) {
        const wallet = availableWallets.find(w => w.name === requestedWalletName)
        if (wallet && wallet.provider) {
          targetProvider = wallet.provider
          selectedWalletName = wallet.name
        } else if (requestedWalletName === 'WalletConnect') {
          throw new Error('WalletConnect not implemented yet - use browser extension wallets for now')
        }
      } else {
      ...
      if (!targetProvider) {
        throw new Error(`No ${requestedWalletName || 'compatible'} wallet detected. Please install a Web3 wallet.`)
      }
      ...
      console.log(`Successfully connected to ${selectedWalletName}:`, accounts[0]) // DEBUG: will remove later

      // Network check (don't throw error, just warn)
      if (Number(network.chainId) !== ARBITRUM_LOCAL_CHAIN_ID) {
        console.warn(`Connected to chain ${network.chainId}, expected ${ARBITRUM_LOCAL_CHAIN_ID}`) // DEBUG: will remove later
      }
  ...
  useEffect(() => {
    const checkPreviousConnection = async () => {
      ...
      try {
        ...
        console.log('Checking previous connection:', { accounts: accounts.length, wasConnected, savedWallet }) // DEBUG: will remove later
